...
import { NetworkUpdateRequest } from '../models/network-update-request';
...
export class AdvancedSettingsService {
  ...
  getAdvancedSettings(): Observable<AdvancedSettingsResponse> {
    const data = this.http.get<AdvancedSettingsResponse>(
      `${this.sensorApiRoot}/GetAdvancedSettings`,
    );
    return data ?? [];
  }
  ...
  updateNetowrkSettings(networkSettings: NetworkUpdateRequest): Observable<void> {
    return this.http.patch<void>(`${this.sensorApiRoot}/UpdateNetworkSettings`, networkSettings);
  }
}
